...
import jack.changgou.vo.BCrypt;
import jack.changgou.vo.JwtUtil;
...
public class UserController {
    ...
    public Result login(String username, String password, HttpServletResponse response){
        ...
        // 对比密码，使用BCrypt加密，因此这里使用该算法进行校验
        if(BCrypt.checkpw(password, user.getPassword())) {
            // 登陆成功，创建用户令牌
            Map<String, Object> tockenMap = new HashMap<String, Object>();
            tockenMap.put("role", "USER");
            tockenMap.put("success", "SUCCESS");
            tockenMap.put("username", username);
            String jwt = JwtUtil.createJWT(UUID.randomUUID().toString(), JSON.toJSONString(tockenMap), null);
        ...
        // 登陆失败
        return new Result(true, StatusCode.ERROR,"登陆失败");
    }
